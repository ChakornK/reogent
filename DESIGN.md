...
- **Course chips**: Search results, placed courses, and drag ghosts use the same raised two-row chip. The first row contains a plain-text course code and a separate action group: an icon-only details button, a fixed-width Add or Move action, and a reserved remove slot. Course code and title share one left text edge. Details and remove use matching square buttons with centered icons; the action group uses uniform 4px gaps. Add and Move text align to the start of their fixed action slot. Course text stays outside buttons and serves as a drag surface. Button hit areas retain their full height. Search results keep the unused remove slot invisible; ghosts retain the source markup under `inert` and `aria-hidden`. The second row contains a single-line title and fixed-width credits. Dragging preserves the source width without scaling the chip. Year columns keep an 18rem minimum width so course codes and actions remain readable. Add and Move reveal a native term selector that omits co-op terms and disables the current move destination; both use the same undoable store operations as drag and drop. Pointer drag retains anchored spring and velocity tilt, with immediate anchored positioning for reduced-motion users.
