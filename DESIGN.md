...
- **Course chips**: Search results, placed courses, and drag ghosts use the same raised two-row chip. The first row contains the course-code details button, a fixed-width Add or Move action, and a reserved remove slot. The details button has zero inline padding so its code aligns with the title below; Add and Move text align to the start of their fixed action slot. Button hit areas retain their full height. Search results keep the unused remove slot invisible; ghosts retain the source markup under `inert` and `aria-hidden`. The second row contains a single-line title and fixed-width credits. Dragging preserves the source width without scaling the chip. Year columns keep an 18rem minimum width so course codes and actions remain readable. Add and Move reveal a native term selector that omits co-op terms and disables the current move destination; both use the same undoable store operations as drag and drop. Pointer drag retains anchored spring and velocity tilt, with immediate anchored positioning for reduced-motion users.
